...
vector<double> Qfunction(vector <double> SNR)
{
    vector <double> Qvalue;
    double po;
    for (int k =0; k<SNR.size(); k++){
        po = Qfunc(sqrt(2*SNR[k]));
...
int main(){
    ...
    for(int i =0; i<SNR.size(); i++){
        energyOfSymbol.insert(energyOfSymbol.end(), N_o*SNR[i]);
    }
    ...
    datafile(SNR, Prob_error);
    vector<double> qvalue = Qfunction(SNR);
    qvalueInFile(SNR, qvalue);

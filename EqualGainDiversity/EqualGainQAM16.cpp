...
int main(){
    ...
    for(int step = 0; step<energyOfSymbol.size(); step++){
        ...
        if(Base==4){
            a = sqrt(energyOfSymbol[step]/10);
        }
        if(Base == 3){
            a = sqrt(energyOfSymbol[step]/6);
        }
        ...
        transmittedSymbol1 = qam1.TransSymbol_colVect1(Symbols, EnergyComponent, AngleComponent);
        transmittedSymbol2 = qam1.TransSymbol_colVect2(Symbols, EnergyComponent, AngleComponent);

        //Rayleigh distributed fadding coffcients
        for(int i =0; i<L; i++){
            realGaussian = qam1.GnoiseVector(mean, stddevRayleigh, one_million);
            imziGaussian = qam1.GnoiseVector(mean, stddevRayleigh, one_million);
        ...
        for(int i=0; i<L; i++){
            gnoise = qam1.GnoiseVector(mean, stdnoise, one_million);
        ...
        //ML decoder
        decodedBits = qam1.DecisionBlock_M16(AfterCombining1, AfterCombining2, a, SumOfHIs1, SumOfHIs2);

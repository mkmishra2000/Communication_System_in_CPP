...
// Topic:- Performance of equal gain combiner using BPSK in Rayleigh fadding channel
/////////////////////////////////////////////////////////////////////////////////////////////////
...
int main(){
    ...
    vector<double> imziGaussian;
    vector<double> RayleighNoise;
    vector<double> SumofHis1;
    vector<double> SumofHis2;
    ...
    for(int step = 0; step<energyOfSymbol.size(); step++){
        ...
        transmittedSymbol1 = SignalVectors1(Symbols, energyOfSymbol[step], M);
        transmittedSymbol2 = SignalVectors2(Symbols, energyOfSymbol[step], M);

        //Rayleigh distributed fadding coffcients
        for(int i =0; i<L; i++){
            realGaussian = GnoiseVector(0.0, stddevRayleigh, transmittedSymbol1.size(), one_million);
            imziGaussian = GnoiseVector(0.0, stddevRayleigh, transmittedSymbol1.size(),one_million);
        ...
        for(int i =0; i<L; i++){
            realGaussian = GnoiseVector(0.0, stddevRayleigh,transmittedSymbol2.size(), one_million);
            imziGaussian = GnoiseVector(0.0, stddevRayleigh, transmittedSymbol2.size(), one_million);
        ...
        for(int i=0; i<L; i++){
            gnoise = GnoiseVector(0.0, stdnoise, transmittedSymbol1.size(), one_million);
        ...
        //Gaussian noise (white noise)
        for(int i=0; i<L; i++){
            gnoise = GnoiseVector(0.0, stdnoise, transmittedSymbol2.size(), one_million);
        ...
        //combining operation
        AfterCombining1 = RawWiseSum(ReceiveMat1);

        //combining operation
        AfterCombining2 = RawWiseSum(ReceiveMat2);

        // Estimating the Hi's
        SumofHis1 = RawWiseSum(RaleighMat1);
        SumofHis2 = RawWiseSum(RaleighMat2);

        //ML decoder
        decodedBits = decisionBlock(AfterCombining1, AfterCombining2, energyOfSymbol[step],SumofHis1, SumofHis2 , M);

        //Count error
        counterror = errorCalculation(Symbols, decodedBits);

        //Probability of error
        P_error = counterror/Symbols.size();
    ...
    char NameofFile1[30] = "EGCMPSK(M=4)Ltest";

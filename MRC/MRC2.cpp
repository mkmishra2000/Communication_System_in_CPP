...
double ProbabilityOferror(double SNR, double L)
{
    ...
    double fun, interg, sinF;
    vector<double> y_axis;
    for(int i =0; i<num; i++){
        sinF = pow(sin(currentValue), 2);
        fun = 1/(1+(SNR/(2*sinF)));

        y_axis.insert(y_axis.end(), pow(fun, L));

        currentValue = currentValue + width;
    }
    interg = DefiniteIntegration(y_axis, b, a, num);
    pe = interg/pi;
...
int main(){
    ...
    char NameofFile1[30] = "MRCL8";         //Name of file

    char NameofFile2[30] = "MRCL8Error";    //Name of file
